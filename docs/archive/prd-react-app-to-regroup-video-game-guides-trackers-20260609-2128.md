...
Source Brainstorm: brainstorm-react-app-to-regroup-video-game-guides-trackers-20260609-2036.md
...
**Section Status:** ✅ APPROVED (2026-06-09)
...
*AC: from cold open to current step visible requires 0 navigation actions; works offline.*

**P2 — Check off progress in bursts.** As Player, I mark several completed steps in one interaction burst.
*AC: marking N consecutive steps takes ≤ N+1 taps; state persists immediately (survives closing the tab).*

**P3 — Never miss a missable.** As Player, I am warned about missables *before* I pass the point of no return.
*AC: any missable within the upcoming section is visibly flagged on the current-step view; dismissing a missable warning is an explicit action.*
...
*AC: genre widget for the current chapter reachable in ≤ 2 taps from current step; returning to current step is 1 tap.*

**P5 — Sync RA achievements.** As Player, I press Sync and the achievements I earned in-game get checked in the guide.
*AC: pressing Sync marks all RA-unlocked achievements mapped in the guide as done; a receipt reports counts (newly marked / already done / unmapped); first-ever sync imports full RA history.*

**P6 — Track a grind.** As Player, I count repetitive attempts (soft resets, deaths) without leaving my current view.
*AC: an increment action is available from the current-step view in 1 tap once a counter exists for that context.*

**P7 — Clean up post-game.** As Player, after credits I switch to cleanup and see remaining tasks grouped by location.
*AC: cleanup view lists all unchecked items grouped by region/chapter; steps I explicitly skipped earlier appear in it.*
...
*AC: a new genre arrangement renders correctly with zero changes outside data files.*

**E2 — Compile in passes.** As Editor, I run compiler passes (spine → widgets → RA mapping → QA) that each produce a reviewable artifact.
*AC: each pass outputs a layer file + a report (row counts, anomalies, confidence flags); a failed pass never modifies an approved layer.*
...
*AC: every flagged row shows its source reference; flag count is visible per layer.*

**E4 — Spot-check randomly.** As Editor, I request N random rows from a layer with their source excerpts for verification.
*AC: spot-check picks uniformly at random; verdicts (pass/fail + note) are recorded.*

**E5 — Approve or reject a layer.** As Editor, I approve a layer (locking it) or reject it with a note that feeds the recompile.
*AC: approved layers are immutable to later passes; rejection notes are stored with the layer; unapproved guides are visibly marked unfinished in the app.*
...
*AC: each of the five migrated guides renders with content equivalent to its HTML original; previously checked items remain checked.*
...
**Section Status:** DRAFT — awaiting approval

---

## 6. Data Model

*The schema is the product. Entities, fields, and invariants any implementation must honor — exact serialization to be designed schema-first at implementation start.*
...
**Section Status:** ✅ APPROVED (2026-06-10)
